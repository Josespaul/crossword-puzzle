            ...
            */
            checkAnswer: function(e) {
                ...
                if (valToCheck === currVal) {
                    ...
                    //alert(puzz.data.length)
                    //alert(JSON.stringify(puzz.data[activePosition].position))
                    //alert(JSON.stringify(puzz.data))

                    puzz_check[puzz.data[activePosition].position - 1] = true

                    var complete_flag = false;
                    for (let i = 0; i < puzz.data.length; i++) {
                        console.log("Game Complete Check - i,puzz_check[i]" + i + ',' + puzz_check[i])
                        if (puzz_check[i] == false) { complete_flag = false; break; }
                        complete_flag = true;
                    }
